...
export const VALID_RANGES = new Set(["1M", "3M", "6M", "1Y", "2Y"]);
export const VALID_INTENTS = new Set(["price", "phase", "state", "risk", "summary"]);
...
function normalizeRange(value) {
  const range = String(value || "1Y").trim().toUpperCase();
  return VALID_RANGES.has(range) ? range : "1Y";
}

function normalizeIntent(value) {
  const intent = String(value || "summary").trim().toLowerCase();
  return VALID_INTENTS.has(intent) ? intent : "summary";
}
...
export function toSkillPayload(result) {
  return {
...
export async function runStockAnalysis(input = {}) {
  const ticker = normalizeTicker(input.ticker);
  const rangeLabel = normalizeRange(input.range);
